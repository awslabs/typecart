...
module N {
  ...
  // test for LambdaExpr
  // we need to identify why eclosing barckets are missing for a lambda expression in the output "(n: int) => New.N.INT5"
  // Line 185 of Expect.dfy
  ...
  // the tool doesnot check for multiset, we should implement it
  const INT1 := 1
  ...
  // test for NegationExpression and UnaryOpExpr
  // should be cover by UnaryExp, but its included for completeness
  const NegInt1 := -1
  type SetNegationExpression = s : seq<int> | |s| == - NegInt1
    witness [0]
  
  // test for ParensExpression
  // cover by other cases, only there for completeness
  const Int1 := 1
  ...
  // test for StmtExpr
  // expressions in statements also need to be prefixed, not implemented for the time being
