        ...
        *)
        ...
        member this.declDefault(ctx: Context, d: Decl) : Decl list =
            ...
            | Class (n, isT, tpvs, ps, ds, m) ->
                ...
                let psT =
                    List.map (fun p -> this.classType (ctx, p)) ps
            ...
            | Method (methodType, n, tpvs, ins, outs, modifies, reads, decreases, b, isG, isS, isO, m) ->
                ...
                let outsT =
                    match outs with
                    | OutputSpec (ds, cs) ->
                        OutputSpec(this.localDeclList (ctxIns, ds), this.conditionList (ctxIns, cs))
                ...
                let bodyCtx =
                    (ctxIns.add outs.namedDecls).enterBody ()

                let bT =
                    Option.map (fun b -> this.expr (bodyCtx, b)) b

                [ Method(methodType, n, tpvs, insT, outsT, modifiesT, readsT, decreasesT, bT, isG, isS, isO, m) ]
        ...
        member this.exprDefault(ctx: Context, expr: Expr) : Expr =
            ...
            | EMapComp (lds, p, tL, tR) ->
                ...
                let pTL =
                    Option.map (fun e -> this.expr (ctxE, e)) tL
            ...
            | ELet (v, x, o, df, bd) ->
                ...
                ELet(this.localDeclList (ctx, v), x, o, dfT, this.expr (ctxI, bd))
    ...
    *)
    ...
    type CheckReferences() =
        ...
        override this.tp(ctx: Context, t: Type) =
            ...
            | TVar n ->
                let c =
                    (List.filter (fun (a, _) -> a = n) ctx.tpvars)
                        .Length
        ...
        override this.expr(ctx: Context, e: Expr) =
            ...
            | EVar (name = n) ->
                let c =
                    Utils.listCount (List.map localDeclName ctx.vars, n)
