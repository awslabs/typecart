...
module Analysis =
    ...
    /// Returns a set of paths that is directly used by a input set of decls (represented as paths).
    type GatherPathsUsedByPaths(start: Set<Path>) =
        inherit Traverser.Identity()
        
        let mutable result: Set<Path> = Set.empty
        let mutable inStart: int = 0 // record the current number of decls in the start set in the call stack
        
        member this.updateInStart(p: Path, enter: bool) =
            if Set.contains p start then
                if enter then
                    inStart <- inStart + 1
                else
                    inStart <- inStart - 1
        ...
        override this.decl(ctx: Context, d: Decl) : Decl list =
            ...
            | Method (_, n, _, _, _, _, _, _, _, _, _, _, _)
            | ClassConstructor (n, _, _, _, _, _) ->
                this.updateInStart(ctx.currentDecl.child(n), true)
            | _ -> ()
            
            let result = this.declDefault(ctx, d)
            ...
            | Method (_, n, _, _, _, _, _, _, _, _, _, _, _)
            | ClassConstructor (n, _, _, _, _, _) ->
                this.updateInStart(ctx.currentDecl.child(n), false)
            | _ -> ()
