...
module TypeInjections =
    ...
    let private genInj (s: string) : MemberDecl Option =
        ...
        | _, _ ->
            System.Console.WriteLine("Only datatypes, newtypes, type synonyms, and subset types supported")
            None
    ...
    let genOuter includeLemmas : ModuleDecl =
        ...
        List.iter
            (fun (s: string) ->
                match genInj s with
                | None -> ()
                | Some funDef -> members.Add(funDef))
